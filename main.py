...
class Zefoy:
	def __init__(self):
		...
		self.captcha_1 = None
		self.url = 'https://www.tiktok.com/@0noak/video/7338050474748185862'
		self.service = ''
		self.proxy_ = ''
		self.proxy = None
		self.captcha_auto_solve = True
	...
	def send_captcha(self, new_session = False):
		...
		if self.get_captcha(): return (True, 'The session already exists')
	...
	def get_table(self, i = 1):
		...
		while True:
			...
			else: self.send_captcha();time.sleep(2)
	...
	def find_video(self):
		...
		while True:
			...
			if 'Session expired. Please re-login' in self.video_info: self.send_captcha(); return (False,)
	...
	def use_service(self):
		...
		if 'Session expired. Please re-login' in res: self.send_captcha(); return ""
		...
		elif 'Please try again later. Server too busy' in self.video_info: exit("Busy")
	...
	def get_video_id(self, url_ = None, set_url=True):
		...
		if 'author' not in json_: print(f'{self.url}| invalid URL | Replace it'); return False
...
if os.path.exists('config.json') is False: open('config.json','w',encoding='utf-8',errors='ignore').write(json.dumps({'url':input('- Enter URL : '),'service':'Views','proxy':None,'captcha_auto_solve':False},indent=4))
